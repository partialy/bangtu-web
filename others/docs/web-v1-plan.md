...
- 顶部：可复用 `HeaderBar`，居中展示平台名称，右侧放安全/菜单入口。
- 搜索：地区选择放搜索框左侧，搜索框保持小号、紧凑。
- 轮播：读取旧小程序 `tp_ad` 中 `open=1`、`ad_type=1` 的广告图，只读展示。
- 公告：展示最新置顶公告，点击进入公告详情；更多进入公告列表。
- 快捷入口：信息、项目、商家、发布、订单，压缩成 4 到 5 个图标。
- 首屏主体：置顶信息 + 最新信息流。
- 底部 Tab：首页、商家、发布、项目、我的。

页面壳要求：

- 详情页复用 `ActivityShell`、`HeaderBar`、`BackButton`，按手机 App Activity 思路组织。
- `HeaderBar` 支持左中右插槽，内容区域由页面传入，方便后续安卓 WebView 适配。
- 页面进入动画使用 Framer Motion 从右侧滑入。

公告增强：

- `web_notice` 支持 `summary`、`content_type`、`popup_enabled`。
- 后台发布公告可选择首页弹窗；前端使用 `localStorage` 记录公告 ID，避免重复弹窗。
- 公告详情支持 HTML 片段，富文本图片必须限制在容器宽度内。
- 后台公告富文本编辑器后续接入；图片通过七牛前端直传后插入 HTML。
